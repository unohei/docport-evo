...
function buildTimelineEntries(docs, myHospitalId) {
  ...
  for (let i = 0; i < chronological.length; i++) {
    ...
    const prevReceived =
      prevDoc != null && prevDoc.to_hospital_id === myHospitalId;
  ...
  for (const doc of docs) {
    ...
    if (doc.assigned_department) {
      entries.push({
        kind: "action",
        subtype: "assign",
        label: `アサイン：${doc.assigned_department}`,
        doc,
      });
    }
...
function TimelineActionEntry({ entry }) {
  const c = ACTION_COLORS[entry.subtype] ?? {
    text: DP.textSub,
    dot: DP.border,
    bg: "transparent",
  };
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {entries.map((entry) =>
          entry.kind === "action" ? (
            <TimelineActionEntry
              key={`action-${entry.doc.id}-${entry.subtype}`}
              entry={entry}
            />
          ) : (
            <TimelineDocEntry
              ...
              onClick={() =>
                onDocSelect(selectedDoc?.id === entry.doc.id ? null : entry.doc)
              }
            />
          ),
        )}
...
export default function ConversationDetailPane({
  ...
  const peerHospitalName = group.peerAssignedHospitalId
    ? nameOf(group.peerAssignedHospitalId)
    : null;
  const currentDisplayLabel = senderCurrentLabel(
    group.currentStatus,
    group.peerAssignedDept,
    peerHospitalName,
  );

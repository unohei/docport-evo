...
export default function DetailPane({
  ...
  useEffect(() => {
    ...
    fetchPreviewUrl(doc)
      ...
      .catch((e) => {
        console.error("[DetailPane] fetchPreviewUrl failed:", e?.message);
        if (!cancelled) {
          setInlineUrl("");
      ...
      >
        ...
        {senderStatus && (
          <div
            style={{
              ...
              color: (
                SENDER_STATUS_COLORS[senderStatus.level] ??
                SENDER_STATUS_COLORS.pending
              ).text,
              background: (
                SENDER_STATUS_COLORS[senderStatus.level] ??
                SENDER_STATUS_COLORS.pending
        ...
        {(canAssign || canComplete) && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            {canComplete && (
              <ActionButton onClick={() => setArchiveOpen(true)}>
                完了にする
              </ActionButton>
            )}
          </div>
